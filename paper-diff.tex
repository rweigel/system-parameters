...
%DIF ADD paper.tex               Wed Jul 10 14:13:59 2019
\usepackage{apacite}
%DIF 3c3
...
The 1-second-cadence GIC data used span a total of 35 days across 10 intervals that start and end at midnight Japan Standard Time (JST) and are listed in Table~\ref{intervals}. These data \DIFaddbegin \DIFadd{along with a map showing the location of the transformer and the connected 187~kV power line }\DIFaddend have been presented previously in \citet{Watari2009} and \cite{Watari2015}. These time intervals are those for which GIC data was made available with the exception of three days for which there were obvious problems with either the electric field or GIC measurements. The GIC dataset includes 1~s raw data and 1~Hz low-pass-filtered raw measurements. The results presented here are for the 1~Hz low-pass-filtered GIC measurements\DIFdelbegin %DIFDELCMD < 
...
%DIFDELCMD < \hline
%DIFDELCMD < %%%
\DIFdel{Start Date }%DIFDELCMD < & %%%
\DIFdel{End Date}%DIFDELCMD < \\
%DIFDELCMD < \hline
%DIFDELCMD < %%%
\DIFdel{2006-04-03 }%DIFDELCMD < & %%%
\DIFdel{2006-04-10}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{2006-07-25 }%DIFDELCMD < & %%%
\DIFdel{2006-07-29}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{2006-08-05 }%DIFDELCMD < & %%%
\DIFdel{2006-08-08}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{2006-08-19 }%DIFDELCMD < & %%%
\DIFdel{2006-08-21}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{2006-11-07 }%DIFDELCMD < & %%%
\DIFdel{2006-11-12}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{2006-11-28 }%DIFDELCMD < & %%%
\DIFdel{2006-11-29}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{2006-12-01 }%DIFDELCMD < & %%%
\DIFdel{2006-12-14}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{2006-12-15 }%DIFDELCMD < & %%%
\DIFdel{2006-12-15}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{2007-11-18 }%DIFDELCMD < & %%%
\DIFdel{2007-11-20}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{2007-11-22 }%DIFDELCMD < & %%%
\DIFdel{2007-11-22}%DIFDELCMD < \\
%DIFDELCMD < \hline
...
Several modifications to this basic procedure were considered. Robust regression \citep{Egbert1986} with a Huber weighting constant of $1.345$ and an upper limit of 50 iterations and bi-square weighting with a constant of $4.685$ (both constants provide 95\% efficiency for \DIFdelbegin \DIFdel{normally distributed }\DIFdelend \DIFaddbegin \DIFadd{gaussian-distributed }\DIFaddend errors), both with no hard-cutoff step; time domain pre-whitening and tapering; and weighting the frequencies around each evaluation frequency using Parzen window weights. When using these additional steps individually, we find that the conclusions made regarding the differences each model's prediction ability do not change because the differences in each model's prediction performance are within the calculated uncertainty of the used method. Transfer function \DIFdelbegin \DIFdel{parameter values }\DIFdelend \DIFaddbegin \DIFadd{coefficients }\DIFaddend found using these modifications are generally within the error bars of those for the used procedure when the signal to noise ratio is above $\sim$3 (corresponding to periods \DIFdelbegin \DIFdel{higher than $\sim30$}\DIFdelend \DIFaddbegin \DIFadd{longer than $\sim$30}\DIFaddend ~s; At periods below $\sim$30~s, the electric field measurements are significantly influenced by the instrument response characteristics~\citep{Fujii2015}, aliasing, and the computed values are also expected to be significantly biased due to \DIFdelbegin \DIFdel{low signal to noise }\DIFdelend \DIFaddbegin \DIFadd{a low signal-to-noise }\DIFaddend ratio.)
...
\noindent
\DIFadd{where $GIC_m(t)$ is the model prediction of model $m$ given in Table~\ref{results} at time step $t$, $t_o=600$, and $t_f=85800$, so that sum is over one day of one-second-cadence measurements excluding the first and last 10~minutes}\DIFaddend .
...
Figure~\ref{Phi} shows the transfer function phases (that for Model~1 is 0$^o$ for positive \DIFdelbegin \DIFdel{parameters }\DIFdelend \DIFaddbegin \DIFadd{coefficients }\DIFaddend and 180$^o$ for negative \DIFdelbegin \DIFdel{parameters }\DIFdelend \DIFaddbegin \DIFadd{coefficients }\DIFaddend by definition). The phases for $z_x(\omega)$ and $b(\omega)$ are offset by 120$^o$ while the phases for $a(\omega)$ and $b(\omega)$ are significantly different. \DIFaddbegin \DIFadd{At $T=12$~h, the phases of $a(\omega)$ and $b(\omega)$ differ by 180 degrees and the associated signs are consistent with those of $\overline{a}_o$ and $\overline{b}_o$. 
}

%DIF > At $T=12$~h, the phases of $|a(\omega)|$ and $|b(\omega)|$ match that of $|\overline{b}_o|$ and $|\overline{a}_o|$ and the ratio of  $|b(\omega)|/|a(\omega)|$ of 1.6 is the closest it gets to that of $|\overline{b}_o|/|\overline{a}_o|$ of approximately 1. However, the absolute values of $a(\omega)$ and $b(\omega)$ are significantly larger than $a_o$ and $b_o$

%DIF > From Figure~\ref{Z} and Figure~\ref{Phi}, it follows that the In Figure~\ref{Phi}, at $T=12$~h, the phase difference between $a(\omega)$ and $b(\omega)$ of 180 degrees is the same as that for $a_o$ and $b_o$. In addition the ratio of $|b(\omega)|/|a(\omega)| =1.6$ is nearest to the frequency independent value predicted by Model~1 of $|\overline{b}_o|/|\overline{a}_o| = 1$.
\DIFaddend 
...
\begin{table}
\caption{\DIFaddFL{Periods associated with evaluation frequency bands used for regression and smoothing spectra. }\# \DIFaddFL{is the evaluation frequency number, $N$ is the number of DFT points in the window, and $T_e=1/f_e$. The window range is $T_l$ through $T_h$.}}
\centering
\begin{tabular}{l l l l l}
\hline \\
\# & \DIFaddFL{$N$ }& \DIFaddFL{$T_e$ }[\DIFaddFL{s}] & \DIFaddFL{$T_l$ }[\DIFaddFL{s}] & \DIFaddFL{$T_h$ }[\DIFaddFL{s}] \\
\hline \\
