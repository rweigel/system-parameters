...
%DIF ADD paper.tex               Mon Jul  1 14:59:57 2019
\usepackage{apacite}
%DIF 3c3
...
\DIFadd{The mean-squared-error on day $D$ is
...
\noindent
\DIFadd{where $\overline{GIC(t)}$ is an average over the same time interval. PE }\DIFaddend is a measure of the skill of the model relative to the variance in the data that are predicted and it represents the fraction of the variance in the data that \DIFaddbegin \DIFadd{is }\DIFaddend predicted by the model.
...
\DIFadd{From Figure~\ref{Z} and Figure~\ref{Phi}, it follows that the 
...
\begin{table}
\caption{\DIFaddFL{Periods associated with evaluation frequency windows used for regression and smoothing spectra. }\# \DIFaddFL{is the evaluation frequency number, $N$ is the number of DFT points in the window, and $T_e=1/f_e$. The window range is $T_l$ through $T_h$.}}
\centering
