...
%DIF ADD paper-submitted-2.tex   Thu Aug  1 15:08:17 2019
\usepackage{apacite}
%DIF 3c3
...
\DIFaddend For the transfer function $\boldsymbol{Z}$ in $\mathbf{E}(\omega) = \boldsymbol{Z}(\omega)\mathbf{B}(\omega)$ used in Model~3, the $xx$ and $xy$ components of $\boldsymbol{Z}$ are solved for using
...
\DIFdelbegin \DIFdel{Prediction }\DIFdelend \DIFaddbegin \DIFadd{GIC }\DIFaddend time series were calculated by \DIFdelbegin \DIFdel{convolution }\DIFdelend \DIFaddbegin \DIFadd{multiplication }\DIFaddend of the interpolated transfer function \DIFdelbegin \DIFdel{parameters }\DIFdelend \DIFaddbegin \DIFadd{coefficients }\DIFaddend with the DFT of the 24-hr long and 1~s cadence model inputs and computing the inverse DFT (e.g, for Model~2, \DIFdelbegin \DIFdel{$G_E(f)=a^I(f)E_x(f)+b^I(f)E_y(f)$ }\DIFdelend \DIFaddbegin \DIFadd{$G_E(\omega)=a^I(\omega)E_x(\omega)+b^I(\omega)E_y(\mega)$ }\DIFaddend was calculated, where the superscript $I$ indicates the interpolated value, and then the inverse DFT of \DIFdelbegin \DIFdel{$G_E(f)$ }\DIFdelend \DIFaddbegin \DIFadd{$G_E(\omega)$ }\DIFaddend was calculated to give the \DIFdelbegin \DIFdel{prediction }\DIFdelend \DIFaddbegin \DIFadd{estimate of }\DIFaddend $G_E(t)$ on a 1~s time grid).

Several modifications to this basic procedure were considered. Robust regression \citep{Egbert1986} with \DIFaddbegin \DIFadd{either }\DIFaddend a Huber weighting \DIFaddbegin \DIFadd{using a }\DIFaddend constant of $1.345$ \DIFdelbegin \DIFdel{and an upper limit of 50 iterations and }\DIFdelend \DIFaddbegin \DIFadd{or }\DIFaddend bi-square weighting \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{using }\DIFaddend a constant of $4.685$ (both \DIFaddbegin \DIFadd{with no hard-cutoff step). The commonly used }\DIFaddend constants provide 95\% efficiency for \DIFdelbegin \DIFdel{normally distributed errors), both with no hard-cutoff step; time domain }\DIFdelend \DIFaddbegin \DIFadd{gaussian-distributed errors. Models were also computed using time domain }\DIFaddend pre-whitening \DIFdelbegin \DIFdel{and tapering ; }\DIFdelend \DIFaddbegin \DIFadd{(using a 10th-order Yule-Walker model computed using the model output), time domain tapering (using a Parzen window), }\DIFaddend and weighting the frequencies around each evaluation frequency using Parzen window weights. When using these additional \DIFdelbegin \DIFdel{steps }\DIFdelend \DIFaddbegin \DIFadd{procedures }\DIFaddend individually, we find that the conclusions made regarding the differences each model's prediction ability do not change because the differences in each model's prediction performance are within the calculated uncertainty of the used method. Transfer function \DIFdelbegin \DIFdel{parameter values }\DIFdelend \DIFaddbegin \DIFadd{coefficients }\DIFaddend found using these modifications are generally within the error bars of those for the used procedure when the \DIFdelbegin \DIFdel{signal to noise }\DIFdelend \DIFaddbegin \DIFadd{signal-to-noise }\DIFaddend ratio is above $\sim$3 (corresponding to periods \DIFdelbegin \DIFdel{higher than $\sim30$~s; At periods below }\DIFdelend \DIFaddbegin \DIFadd{longer than $\sim$30~s); at periods shorter than }\DIFaddend $\sim$30~s, the electric field measurements are significantly influenced by the instrument response characteristics~\citep{Fujii2015}, aliasing, and the computed values are also expected to be significantly biased due to \DIFdelbegin \DIFdel{low signal to noise }\DIFdelend \DIFaddbegin \DIFadd{a low signal-to-noise }\DIFaddend ratio.)
...
\DIFadd{The mean-squared-error for model $m$ on day $D$ is
}\begin{equation*}
...
The prediction efficiency, PE, is the ``Case I'' skill score described by \cite{Murphy1988} and is \DIFdelbegin \DIFdel{defined as PE = 1 - variance(Prediction Error)/variance(Predicted); it }\DIFdelend \DIFaddbegin \DIFadd{calculated for day $D$ using
}\begin{equation*}
...
\begin{table}
  ...
  \begin{tabular}{l c c c c}
    ...
    \hline
  \end{tabular}
  \DIFdelbeginFL %DIFDELCMD < %DIFDELCMD < \label{results}%%%
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{resultstable}
...
}\DIFaddend Figure~\ref{Z_MT} and~\ref{Phi_MT} show the parameters for $\mathbf{Z}$ in Model~3. Note that \DIFaddbegin \DIFadd{transfer function shown in Figure~\ref{Z_MT} differs significantly from that of a layered slab conductivity model for which $Z_{xy}=-Z_{yx}$ and $Z_{xx}=Z_{yy}=0$. That }\DIFaddend electric and magnetic field data from \DIFdelbegin \DIFdel{Memambetsu}\DIFdelend \DIFaddbegin \DIFadd{Memanbetsu}\DIFaddend , Japan were also used by \cite{Fujii2015} to compute $\mathbf{Z}$. When using the \cite{Fujii2015} transfer function for Model~3, the PE was significantly lower (0.58 vs. 0.78). \DIFdelbegin \DIFdel{(The \mbox{%DIFAUXCMD
