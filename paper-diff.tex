...
%DIF ADD paper-submitted-2.tex   Fri Aug  2 14:14:04 2019
\usepackage{apacite}
%DIF 3c3
...
\end{linenomath*}
\noindent
and \DIFdelbegin \DIFdel{are computed using contemporaneous measurements }\DIFdelend \DIFaddbegin \DIFadd{were estimated using the Fourier transforms }\DIFaddend of  $GIC(t)$\DIFdelbegin \DIFdel{and $\mathbf{E}(t)$. The method of parameter estimation and generating predictions for Models 2-4 }\DIFdelend \DIFaddbegin \DIFadd{, $E_x(t)$, and $E_y(t)$; the method of coefficient estimation and the generation of time-domain GIC estimates for this and the following models }\DIFaddend are given at the end of this section\DIFdelbegin \DIFdel{and fourier transformed variables are indicated by explicitly listing their dependence on $\omega$}\DIFdelend \DIFaddbegin \DIFadd{.
}
...
\noindent
where, explicitly, the electric field components are \DIFdelbegin \DIFdel{$E'_x$ }\DIFdelend \DIFaddbegin \DIFadd{$E'_x(\omega)$ }\DIFaddend = \DIFdelbegin \DIFdel{$Z_{xx}B_x + Z_{xy}B_y$ and $E'_y = Z_{yx}B_x + Z_{yy}B_y$}\DIFdelend \DIFaddbegin \DIFadd{$Z_{xx}(\omega)B_x(\omega) + Z_{xy}(\omega)B_y(\omega)$ and $E'_y(\omega) = Z_{yx}(\omega)B_x(\omega) + Z_{yy}(\omega)B_y(\omega)$}\DIFaddend .
...
\noindent
In this model, the transfer function components \DIFdelbegin \DIFdel{$z_x$ and $z_y$ }\DIFdelend \DIFaddbegin \DIFadd{$z_x(\omega)$ and $z_y(\omega)$ }\DIFaddend are determined directly from measurements of $GIC(t)$\DIFdelbegin \DIFdel{and $\mathbf{B}(t)$ }\DIFdelend \DIFaddbegin \DIFadd{, $B_x(t)$, and $B_y(t)$ }\DIFaddend and no electric field measurements are used; this model is expected to produce \DIFdelbegin \DIFdel{predictions }\DIFdelend \DIFaddbegin \DIFadd{estimates }\DIFaddend of GIC that are similar to that obtained from Model\DIFaddbegin \DIFadd{~}\DIFaddend 3 because they both use the same input \DIFdelbegin \DIFdel{variable}\DIFdelend \DIFaddbegin \DIFadd{measurements}\DIFaddend .
...
Figure~\ref{Z_MT} and~\ref{Phi_MT} show the \DIFdelbegin \DIFdel{parameters for }\DIFdelend \DIFaddbegin \DIFadd{magnitude and phase for the coefficients of }\DIFaddend $\mathbf{Z}$ in Model~3. Note that \DIFaddbegin \DIFadd{transfer function coefficients shown in Figure~\ref{Z_MT} differ significantly from that of a layered slab conductivity model for which $Z_{xy}=-Z_{yx}$ and $Z_{xx}=Z_{yy}=0$. That }\DIFaddend electric and magnetic field data from \DIFdelbegin \DIFdel{Memambetsu}\DIFdelend \DIFaddbegin \DIFadd{Memanbetsu}\DIFaddend , Japan were also used by \cite{Fujii2015} to compute $\mathbf{Z}$. When using the \cite{Fujii2015} transfer function for Model~3, the PE was significantly lower (0.58 vs. 0.78). \DIFdelbegin \DIFdel{(The \mbox{%DIFAUXCMD
