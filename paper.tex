...
Note that the values of $\overline{a}_o = 73$ A/(V/km) and $\overline{b}_o = -72$ A/(V/km) imply that for a given electric field magnitude, GIC is largest at the measurement site when the electric field points in the northwest direction (\cite{Pulkkinen2010} and \cite{Watari2015} also found values of $a_o>0$ and $b_o<0$). However, as shown in Figure~\ref{map}, the 187 kV line which terminates at the Memanbetsu substation runs in the northeast direction, and hence the dominant contribution to the measured GIC would be from currents flowing in this line. If the electric field is the same at all points on the line, GIC in the grounded transformers at Memanbetsu substation should be the largest when the electric field points in the northeast direction.
...
Note that in this discussion, we have attempted to address independently the contribution of spatial variations in the electric field magnitude and direction and spatial variations in the electric field spectra, but generally these variations do not occur independently as they are both caused by horizontal variations in the ground conductivity. Thus, the finding that the angle of the electric field that produces maximum GIC differs significantly from the angle of the 187~kV power line (for both Model~1 and Model~2) is likely to be consistent with the frequency dependence of $a$ and $b$ found in Model~2. Testing this claim would require a full network analysis an knowledge of the ground conductivity throughout the network.

%We have argued above that a spatial variation in the spectra of the electric field can explain the frequency dependence of $a$ and $b$. There is also evidence that the electric field varies along 

%A consideration of the ratio of $b_o/a_o$ suggests that the ground conductivity varies significantly along the length of the line. As shown in Figure~\ref{map}, the 187~kV line runs in a direction that is approximately 45$^o$ east of north.  Assuming negligible GIC contributions from other unknown parts of the network and an electric field that is uniform along the length of the line, the equation for Model~1, $GIC(t) = a_oE_x(t) + b_oE_y(t)$, where $x$ is northward and $y$ is eastward,

%one would expect the values of $a_o$ and $b_o$ to be such that $tan^{1}b_o/a_o$ is approximately 45$^o$ east of north . 
