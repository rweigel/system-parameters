...
%A preliminary investigation suggests that the cause is due to several factors: we have found using continuous one-month intervals prior to 2006 to compute $\mathbf{Z}$, our calculated transfer function (using all of the modifications described in Section~3) matches the \cite{Fujii2015} transfer function to within $\pm X$\% except at low periods, which is likely due to the fact that we did not use a remote reference. After 2006, our calculated transfer functions differ significantly. In addition, we have found that the calculated transfer function is highly sensitive to whether or not stacking was used. Due to the large gaps between and the varying length of data segments, transfer functions were computed using one day of data and then averaged. We have found that if a continuous set of measurements over approximately 21 days is used to compute a single transfer function, the result is significantly different than if 21 transfer functions were computed and then averaged. 

%As a result, the transfer function parameters shown in Figure~\ref{Z_MT} and~\ref{phi_MT} should be considered a 
